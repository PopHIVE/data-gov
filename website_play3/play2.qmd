...
p2 <- usmap::plot_usmap(regions='counties', data=d1,values='percent_visits_combined' )   +
   labs(title = paste0('RSV/flu/COVID ED visits ', dates[i])) + 
    theme(panel.background = element_rect(color = "white", fill = "white")) +
    scale_fill_stepsn('Virus', breaks= seq(0,10, by=1), limits = c(0,10),
                      colors=viridis_pal()(10),
                      guide = guide_colorsteps(even.steps = FALSE))
